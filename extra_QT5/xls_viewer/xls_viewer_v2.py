...
class DataApp(QWidget):
    def __init__(self):
        ...
        self.vertical_line = None  # Référence pour la ligne verticale
    ...
    def plot_data(self):
        if not self.data_frame.empty:
            self.figure.clear()
            self.vertical_line = None  # Reset la référence de la ligne verticale
    ...
    def on_plot_click(self, event):
        if event.inaxes:
            ...
            # Supprimer l'ancienne ligne verticale si elle existe
            if self.vertical_line is not None:
                self.vertical_line.remove()

            # Ajouter une nouvelle ligne verticale verte pointillée
            ax = event.inaxes
            x_closest = x_data.iloc[closest_indices]
            self.vertical_line = ax.axvline(x=x_closest, color='green', linestyle='--', linewidth=2, alpha=0.8)
